...
from trimero.systems.rb_krb_polar.rb_defects import energy_rb, neighbor_levels
...
def rydberg_diagonal(
    block: QuantumBasisBlock, n_manifold: int = 24, n_s: int = None,
    delta0_ns: float = None,
) -> np.ndarray:
    """
    Diagonal de H_a en el orden del bloque: energías del Rb* sin perturbar.

    Manifold cuasi-degenerado (n, l≥3) más los TRES niveles vecinos
    individuales (n+1)d, (n+2)p, (n+3)s — l = 2, 1, 0 respectivamente — tal
    como los enumera Aguilera-Fernández et al., J. Phys.: Conf. Ser. 635,
    012023 (2015). El mapa l→n sale de `rb_defects.neighbor_levels`, no se
    escribe a mano. `n_s` permite forzar el nivel l=0 (compatibilidad).
    Las energías salen de `atom.Atom.E_Rb()` (defectos cuánticos), no se
    duplican aquí.

    `delta0_ns` permite usar el δ₀(ns) que reproduce la Tabla I de
    González-Férez 2015 en vez del de `rb_atom.py` (ver systems/rb_krb_polar/rb_defects.py).
    ...
    for k, (l, _, _, _) in enumerate(block.states):
        if l not in cache:
            n = levels.get(l, n_manifold)
            # delta0_ns=None -> energy_rb delega en Atom.E_Rb() sin cambios.
            # Ver systems/rb_krb_polar/rb_defects.py para por qué existe el override.
