...
| 4. curva BOP completa regenerada | **NO hecho** — el barrido cuesta ~165 s y se cortó por límite de uso. Ejecutar `poetry run python scripts/run_bop_curve.py`; la versión anterior está guardada en `plots/bop_curve_MJ0_linear.npz` para comparar |
...
1. Regenerar la curva BOP completa con `p_interpolation="inverse"` y comparar
   con `plots/bop_curve_MJ0_linear.npz` (punto 4, no ejecutado).
2. **Valorar en serio la corrección de rango efectivo (Omont)**: ya no es
   opcional para la región de la resonancia, es la única vía para obtener una
   profundidad finita y físicamente defendible.
3. Conseguir `A_p(k)` tabulado de una fuente independiente para acotar la
   magnitud cerca del polo.
