...
from trimero.systems.rb_defects import DELTA0_NS_PAPER as D0

GHZ = HZ_PER_HARTREE / 1.0e9
E_MAN = -0.5 / 24.0**2            # manifold n=24 + KRb(N=0): cero de energía
N_CONTEXT = 60
...
HMOL = ChargeDipoleHamiltonian(electron_field=RydbergElectronField(RADIAL))
FERMI = FermiPseudopotential(RADIAL, SCAT, delta0_ns=D0)
E_RYD = rydberg_diagonal(BLOCK, delta0_ns=D0)
IS_MANIFOLD = np.array([st[0] >= 3 for st in BLOCK.states])


def solve(R):
    H = np.diag(E_RYD) + HMOL.build(BLOCK, R) + FERMI.build(BLOCK, R)
...
def parse_args():
    ...
    ap.add_argument("--reuse", action="store_true",
                    help="reutiliza plots/bop_curve_MJ0.npz en vez de rebarrer R")
    ...
    ap.add_argument("--out", default="plots/bop_curve_MJ0.png")
...
def resonance_flags(R_array, ap_threshold):
    ...
    for R in R_array:
        try:
            _, A_p = FERMI.scattering.scattering_pair(
                R, n_star_of_l(3, D0), n_star_of_l(3, D0))
        except ValueError:
            A_p = 0.0
...
def main():
    ...
    np.savez("plots/bop_curve_MJ0.npz", R=R, E=E, y=y, index=idx, context=ctx)
    print("  datos en plots/bop_curve_MJ0.npz")
    print("=" * 84)
